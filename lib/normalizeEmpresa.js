...
function normalizeCnae(value) {
  if (value === null || value === undefined || String(value).trim() === '') return null;
  const raw = String(value).trim();
  const formatted = raw.match(/(\d{2})[.\s]?(\d{2})[-.\s]?(\d)[/.\s]?(\d{2})/);
  if (formatted) return `${formatted[1]}${formatted[2]}${formatted[3]}${formatted[4]}`;

  const onlyDigits = digits(raw);
  if (onlyDigits.length >= 6 && onlyDigits.length <= 7) return onlyDigits.padStart(7, '0');
  return null;
}

function normalizeCnaeDescription(explicitDescription, cnaeValue) {
  const raw = text(explicitDescription, 500) || text(cnaeValue, 500);
  if (!raw || !/[A-Za-zÀ-ÿ]/.test(raw)) return null;
  const withoutCode = raw
    .replace(/\b\d{2}[.\s]?\d{2}[-.\s]?\d[/.\s]?\d{2}\b/, '')
    .replace(/^[\s–—:;|-]+/, '')
    .trim();
  return withoutCode || null;
}

function normalizeSegment(raw, cnae, description) {
  const combined = `${raw || ''} ${description || ''}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  const code = cnae || '';

  if (/condomin/.test(combined) || code === '8112500') return 'Condomínio';
  if (/contabil|contabilidade|contador/.test(combined) || code.startsWith('69206')) return 'Parceiro Contábil';
  if (/construcao|construtora|engenharia civil|obra|edificacao/.test(combined) || /^(41|42|43)/.test(code)) return 'Construção Civil';
  if (/transport|logistic|frete|carga|armazenamento/.test(combined) || /^(49|50|51|52|53)/.test(code)) return 'Transportadora';
  if (/outro/.test(combined) || code || description || raw) return 'Outros';
  return null;
}
...
function normalizeEmpresa(row) {
  ...
  const rawCnaeDescription = pick(indexed, [
    'descricao_cnae_principal',
    'descricao_cnae',
  ...
  const cnae = normalizeCnae(rawCnae) || normalizeCnae(rawCnaeDescription);
  const cnaeDescription = normalizeCnaeDescription(rawCnaeDescription, rawCnae);
  ...
  return {
    value: {
      ...
      segmento: normalizeSegment(rawSegment, cnae, cnaeDescription),
      cnae_principal: cnae,
      descricao_cnae_principal: cnaeDescription,
